...
namespace ACViewer.Render
{
    public class InstanceBatch
    {
        ...
        public void UpdateInstance(int index, Vector3 position, Quaternion orientation)
        {
            if (index < 0 || index >= Instances_Env.Count) return;

            var instance = Instances_Env[index];
            var newPos = position;
            newPos.Z += 0.05f; // match AddInstance offset

            instance.Position = newPos;
            instance.Rotation = new Vector4(orientation.X, orientation.Y, orientation.Z, orientation.W);

            Instances_Env[index] = instance;

            // Rebuild instance buffer with updated instances
            BuildInstanceBuffer();
            BuildBindings();
        }
