...
namespace ACViewer.Render
{
    public class Render
    {
        ...
        public void Init()
        {
            ...
            Effect = new Effect(GraphicsDevice, File.ReadAllBytes(Path.Combine(contentDir, "texture.mgfxo")));
            Effect_Clamp = new Effect(GraphicsDevice, File.ReadAllBytes(Path.Combine(contentDir, "texture_clamp.mgfxo")));
        ...
        public void DrawHUD()
        {
            if (!MainMenu.ShowHUD) return;

            var text = "";
    
            if (ConfigManager.Config != null && ConfigManager.Config.MapViewer != null && ConfigManager.Config.MapViewer.EnableZSlicing)
            {
            ...
            if (!string.IsNullOrEmpty(text) && Font != null && SpriteBatch != null)
            {
