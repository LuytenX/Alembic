...
namespace ACViewer
{
    public class Camera
    {
        ...
        public Vector3 PreviewTarget { get; set; } = Vector3.Zero;
        ...
        {
            PreviewTarget = new Vector3(0, 0, 1);
            Position = new Vector3(ParticleDist, ParticleDist, 1);
            Dir = Vector3.Normalize(PreviewTarget - Position);
        ...
        {
            if (landblock == null || landblock.Landblock == null) return;

            var x = landblock.Landblock.ID >> 24;
            var y = landblock.Landblock.ID >> 16 & 0xFF;

            float height = 0.0f;
            if (landblock.Landblock.Polygons != null && landblock.Landblock.Polygons.Count > 0 && 
                landblock.Landblock.Polygons[0].Vertices != null && landblock.Landblock.Polygons[0].Vertices.Count > 0)
            {
                height = landblock.Landblock.Polygons[0].Vertices[0].Origin.Z;
            }
            
            // Center the camera in the landblock (96.0f offset) to prevent immediate boundary triggers
            Position = new Vector3(x * 192.0f + 96.0f, y * 192.0f + 96.0f, height + 50.0f);

            var lookAt = new Vector3(x * 192.0f + 96.0f, y * 192.0f + 96.0f, height);

            Dir = Vector3.Normalize(new Vector3(0, 1, -0.5f)); // Look forward and slightly down
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                if (Locked) Console.WriteLine("Camera Locked");
                return;
            }
            ...
            bool mouseInPip = false;
            // Check if mouse is over the PiP preview or Minimap
            if (GameView.PreviewMode != ViewMode.Undef)
            {
                var pipViewport = GameView.Instance.GetPipViewport(GameView.GraphicsDevice.Viewport);
                mouseInPip = IsMouseInViewport(mouseState.X, mouseState.Y, pipViewport);
            }
            if (!mouseInPip && GameView.ViewMode != ViewMode.Map)
            {
                var miniViewport = GameView.Instance.GetMiniMapViewport(GameView.GraphicsDevice.Viewport);
                mouseInPip = IsMouseInViewport(mouseState.X, mouseState.Y, miniViewport);
            }

            ProcessKeyboardMovement(keyboardState);
            ...
            // camera speed control - block if in PiP (zoom instead)
            if (!mouseInPip && mouseState.ScrollWheelValue != PrevMouseState.ScrollWheelValue)
            {
            ...
            // LMB handling - block if in PiP (rotation instead)
            if (!mouseInPip && mouseState.LeftButton == ButtonState.Pressed)
            {
                if (GameView.ViewMode == ViewMode.World)
                {
                    if (Picker.EditMode || Picker.PaintMode || Picker.ObjectMode)
                    {
                        var projectionInverse = Matrix.Invert(ProjectionMatrix);
                        var viewMatrix = Matrix.CreateLookAt(Vector3.Zero, Dir, Up);
                        var viewInverse = Matrix.Invert(viewMatrix);
                        var transform = projectionInverse * viewInverse;
                        var nx = mouseState.X * 2.0f / ViewportWidth - 1.0f;
                        var ny = 1.0f - mouseState.Y * 2.0f / ViewportHeight;
                        ...
                        Picker.ApplyRaiseBrush(dir, shift, ctrl);
                    }
            ...
            // RMB handling - ALWAYS allow (passthrough)
        ...
        private void ProcessKeyboardMovement(KeyboardState keyboardState)
        {
        ...
        private bool IsMouseInViewport(int x, int y, Viewport viewport)
        {
            return x >= viewport.X && x <= viewport.X + viewport.Width &&
                   y >= viewport.Y && y <= viewport.Y + viewport.Height;
        }

        public void UpdatePreview(GameTime gameTime, Viewport viewport)
        {
            if (Mouse == null || Locked) return;

            var mouseState = Mouse.GetState();
            
            // Check if mouse is within the PiP viewport
            if (IsMouseInViewport(mouseState.X, mouseState.Y, viewport))
            {
                // Left click drag to orbit
                if (mouseState.LeftButton == ButtonState.Pressed && PrevMouseState.LeftButton == ButtonState.Pressed)
                {
                    var xDiff = mouseState.X - PrevMouseState.X;
                    var yDiff = mouseState.Y - PrevMouseState.Y;

                    // Orbiting logic:
                    // Rotate the Position relative to the PreviewTarget.
                    
                    // Vector from target to camera
                    Vector3 relativePos = Position - PreviewTarget;
                    float dist = relativePos.Length();
                    
                    // Yaw (left/right) - rotate relativePos around Up axis
                    var yawMatrix = Matrix.CreateFromAxisAngle(Up, -0.01f * xDiff);
                    relativePos = Vector3.Transform(relativePos, yawMatrix);
                    
                    // Pitch (up/down) - rotate relativePos around right axis
                    var right = Vector3.Normalize(Vector3.Cross(Up, Vector3.Normalize(relativePos)));
                    var pitchMatrix = Matrix.CreateFromAxisAngle(right, 0.01f * yDiff);
                    relativePos = Vector3.Transform(relativePos, pitchMatrix);
                    
                    // Re-calculate Position and Dir
                    Position = PreviewTarget + Vector3.Normalize(relativePos) * dist;
                    Dir = Vector3.Normalize(PreviewTarget - Position);
                }
                ...
                if (mouseState.ScrollWheelValue != PrevMouseState.ScrollWheelValue)
                {
                    ...
                    Vector3 relativePos = Position - PreviewTarget;
                    if (diff > 0) relativePos *= 0.9f; // Zoom in 10%
                    else relativePos *= 1.1f; // Zoom out 10%
                    
                    Position = PreviewTarget + relativePos;
                }
            }
