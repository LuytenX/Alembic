...
namespace ACViewer
{
    public class MapViewer
    {
        ...
        public void LoadMap()
        {
            ...
            try
            {
                switch (ConfigManager.Config.MapViewer.Mode)
                {
                    case MapViewerMode.PreGenerated:
                        ...
                        string path = System.IO.Path.Combine(baseDir, @"Content\Images\highres.png");
                        Console.WriteLine($"MapViewer: Synchronously loading pre-generated map from {path}");
                        if (System.IO.File.Exists(path))
                        {
                            WorldMap = Image.GetTextureFromBitmap(GraphicsDevice, path);
                            Console.WriteLine("MapViewer: Pre-generated map loaded successfully.");
                        }
                        else
                        {
                            Console.WriteLine($"MapViewer Error: Could not find {path}");
                        }
                        break;

                    case MapViewerMode.GenerateFromDAT:
                        Console.WriteLine("MapViewer: Generating map from DAT files in background...");
                        ...
                        worker.DoWork += (sender, doWorkEventArgs) =>
                        {
                            try
                            {
                                ...
                                Console.WriteLine("MapViewer: Map generation complete.");
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine($"MapViewer Generation Exception: {ex.Message}");
                            }
                        };
            ...
            {
                Console.WriteLine($"MapViewer LoadMap Exception: {ex.Message}");
        ...
        public void Init()
        {
            ...
            // Ensure map is loaded if it's not already
            if (WorldMap == null && ConfigManager.Config.MapViewer.Mode == MapViewerMode.PreGenerated)
                LoadMap();
        }
        ...
        {
            // If map is still generating in background, we might have WorldMap == null here
        ...
        {
            if (WorldMap == null) return Vector2.Zero;
        ...
        public void Draw(GameTime gameTime, bool isPreview = false)
        {
            if (!isPreview)
            ...
            if (WorldMap == null)
            {
                if (!isPreview) GraphicsDevice.Clear(Color.LimeGreen);
                return;
            }

            // Use the provided transform for main view, or identity/custom for PiP
            var transform = isPreview ? Matrix.Identity : Scale * Translate;

            // Use Opaque blend for the map texture itself
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.LinearClamp, DepthStencilState.None, RasterizerState.CullNone, null, transform);
            
            if (isPreview)
            {
                // Draw the whole map stretched to the PiP viewport
                spriteBatch.Draw(WorldMap, new Rectangle(0, 0, GraphicsDevice.Viewport.Width, GraphicsDevice.Viewport.Height), Color.White);
            }
            else
            {
