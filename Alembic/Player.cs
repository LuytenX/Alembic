...
namespace ACViewer
{
    public class Player
    {
        ...
        public void Update(GameTime time)
        {
            ...
            // Handle left mouse button for picker functionality
            if (mouseState.LeftButton == ButtonState.Pressed)
            {
                // Check if control key is held for terrain editing operations
                ...
                var nx = mouseState.X * 2.0f / Camera.ViewportWidth - 1.0f;
                var ny = 1.0f - mouseState.Y * 2.0f / Camera.ViewportHeight;

                var viewportCoordNorm = new Vector3(nx, ny, 1.0f);
                var dir = Vector3.Normalize(Vector3.Transform(viewportCoordNorm, transform));

                if (ctrlPressed && PrevMouseState.LeftButton == ButtonState.Pressed)
                {
                    // Apply terrain editing brush (raise/lower terrain, paint textures)
                    ...
                    // In player mode, make sure to re-add the player to the buffer after terrain changes
                    // since the buffer refresh might remove the player character
                    if (WorldViewer.Instance != null && WorldViewer.Instance.PlayerMode && WorldViewer.Instance.Player != null)
                    {
                        var playerObj = WorldViewer.Instance.Player.PhysicsObj;
                        if (playerObj != null)
                        {
                            // Re-add player to the buffer to ensure it remains visible after terrain edits
                            WorldViewer.Instance.Buffer.ClearBuffer();
                            foreach(var lb in LScape.Landblocks.Values) WorldViewer.Instance.Buffer.AddOutdoor(new R_Landblock(lb as Landblock));
                            WorldViewer.Instance.Buffer.AddPlayer(new R_PhysicsObj(playerObj));
                            WorldViewer.Instance.Buffer.BuildBuffers();
                        }
                    }
                }
                else if (isLeftClick && !wasLeftClick)
                {
