                ...
                for (var x = 0; x < SideCellCount; x++)
                {
                    for (var y = 0; y < SideCellCount; y++)
                    {
                        ...
                        if (cellHasWater)
                        {
                            ...
                            if (cellFullyFlooded)
                            {
                                if (LandCells.TryGetValue(x * SideCellCount + y, out var cell))
                                    cell.WaterType = LandDefs.WaterType.EntirelyWater;
                            }
                            else
                            {
                                if (LandCells.TryGetValue(x * SideCellCount + y, out var cell))
                                    cell.WaterType = LandDefs.WaterType.PartiallyWater;
                                waterblock = false;
                            }
                        }
                        else
                        {
                            if (LandCells.TryGetValue(x * SideCellCount + y, out var cell))
                                cell.WaterType = LandDefs.WaterType.NotWater;
                            waterblock = false;
                        }
                    }
                }
                ...
                for (var y = 0; y < SideCellCount; y++)
                {
                    ...
                    for (var i = 0; i < LandDefs.VertexPerCell; i++)
                    {
                        ...
                        for (var j = 0; j < LandDefs.VertexPerCell; j++)
                        {
                            ...
                            if (LandCells.TryGetValue(cellIdx, out var cell_obj) && cell_obj is LandCell lcell)
                            {
                    ...
                    if (LandCells.TryGetValue(x * SideCellCount + y, out var landCell))
                    {
                        landCell.ID = cellID;
