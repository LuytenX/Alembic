...
namespace ACE.Server.Physics.Common
{
    public class Landblock: LandblockStruct
    {
        ...
        public bool IsInfoModified { get; set; }
        ...
        public void destroy_buildings()
        {
            ...
            Buildings.Clear();
            if (StabList != null) StabList.Clear();
        }
        ...
        {
            var lcoordResult = LandDefs.gid_to_lcoord(cellID);
            if (lcoordResult == null) return null;
            var lcoord = lcoordResult.Value;

            var idx = ((int)lcoord.Y & 7) + ((int)lcoord.X & 7) * SideCellCount;

            if (LandCells.TryGetValue(idx, out var cell))
            {
                if (cell.ID == cellID)
                    return (LandCell)cell;
            }
        ...
        public void init_static_objs()
        {
            ...
            else if (Info != null)
            {
                foreach (var info in Info.Objects)
                {
                    ...
                    obj.DatObject = true;
                    obj.SourceStab = info; // Store the direct reference to the Stab object
                    var position = new Position(ID, new AFrame(info.Frame));
                    var outside = LandDefs.AdjustToOutside(position);
                    var cell = get_landcell(position.ObjCellID);
                    if (cell == null)
